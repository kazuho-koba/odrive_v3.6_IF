...
def main():
    ...
    # トルク制御モードの開始
    ax0.set_vel_by_torque_mode(kv=130, ramp_rate=20, torque_lim=1.8)
    ...
    try:
        ...
        start_time = time.time()  # モータ制御開始時刻
        while time.time() - start_time < 5:
            target_vel = 2              # 目標速度のセット
            dt = time.time()-last_time
            last_time = time.time()
            motorcurrent_ave = ax0.get_motorcurrent_ave(motorcurrent_ave)
            last_torque_cmd, last_vel, err_integ = ax0.set_vel_by_torque(
                target_vel, last_vel, err_integ, last_torque_cmd, motorcurrent_ave, dt)
            print("averaged motor current:" + str(motorcurrent_ave) + "[A],  motor velocity:" + str(
                last_vel) + "[rps],  set torque:" + str(last_torque_cmd) + "[Nm] with: " + str(1/dt) + "[Hz]")

        start_time = time.time()  # モータ制御開始時刻
        while time.time() - start_time < 1:
            target_vel = 0              # 目標速度のセット
            dt = time.time()-last_time
            last_time = time.time()
            motorcurrent_ave = ax0.get_motorcurrent_ave(motorcurrent_ave)
            last_torque_cmd, last_vel, err_integ = ax0.set_vel_by_torque(
